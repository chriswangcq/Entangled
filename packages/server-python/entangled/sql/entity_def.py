...
class SqlEntityDef(BaseEntityDef):
    ...
    def create_table_sql(self, dialect: str = "sqlite") -> str:
        ...
        if composite_pk:
            col_defs = [f.column_ddl_no_pk(normalized) for f in self.fields]
            pk_clause = f"PRIMARY KEY({', '.join(f.name for f in pk_fields)})"
            all_parts = col_defs + [pk_clause] + self.constraints
        else:
            col_defs = [f.column_ddl(normalized) for f in self.fields]
            all_parts = col_defs + self.constraints
        if normalized == "postgres" and "entangled_rowid" not in self.field_map:
            all_parts.append("entangled_rowid bigint generated by default as identity")
        cols = ",\n    ".join(all_parts)
    ...
    def index_sqls(self, dialect: str = "sqlite") -> List[str]:
        ...
        if normalized == "postgres" and "entangled_rowid" not in self.field_map:
            stmts.append(
                f"CREATE INDEX IF NOT EXISTS idx_{self.table}_entangled_rowid "
                f"ON {self.table}(entangled_rowid);"
            )
        return stmts
    ...
    def alter_add_column_sqls(self, existing_cols: List[str], dialect: str = "sqlite") -> List[str]:
        ...
        for f in self.fields:
            if f.name not in existing_cols:
                stmts.append(f"ALTER TABLE {self.table} ADD COLUMN {f.alter_column_ddl(normalized)};")
        if normalized == "postgres" and "entangled_rowid" not in existing_cols and "entangled_rowid" not in self.field_map:
            stmts.append(
                f"ALTER TABLE {self.table} ADD COLUMN "
                "entangled_rowid bigint generated by default as identity;"
            )
        return stmts
