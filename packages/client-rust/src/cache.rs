...
impl Cache {
    ...
    pub fn new(path: &Path) -> Self {
        ...
        let pool = r2d2::Pool::new(manager).expect("Failed to create entity cache database pool");
        let cache = Self { pool };
        cache.init_schema();
        cache
    }
    ...
    pub fn gc_stale(&self, max_age_secs: u64) -> usize {
        ...
        let keys: Vec<(String, i64)> = stmt
            .query_map(params![cutoff], |row: &rusqlite::Row| {
        ...
        if count > 0 {
            tracing::info!(
                "[Cache] GC: evicted {} stale entries (older than {}s)",
                count,
                max_age_secs
            );
        }
    ...
    pub fn keys_for_entity(&self, entity: &str) -> Vec<CacheKey> {
        ...
        let mut stmt = conn
            .prepare_cached("SELECT params_hash FROM entity_meta WHERE entity = ?1")
            .unwrap();
        stmt.query_map(params![entity], |row: &rusqlite::Row| {
    ...
    pub fn db_size(&self) -> u64 {
        ...
        let page_count: i64 = conn
            .query_row("PRAGMA page_count", [], |r: &rusqlite::Row| r.get(0))
            .unwrap_or(0);
        let page_size: i64 = conn
            .query_row("PRAGMA page_size", [], |r: &rusqlite::Row| r.get(0))
            .unwrap_or(4096);
...
mod item_id_tests {
    ...
    fn cache_meta_survives_clear_all() {
        ...
        assert_eq!(
            cache
                .get_meta_value("app_cache_contract_version")
                .as_deref(),
            Some("2"),
        );
